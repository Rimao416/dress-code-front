...
const ProductInfo: React.FC<ProductInfoProps> = ({
    ...
    .reduce((acc, variant) => {
      ...
      if (!existingColor) {
        acc.push({ 
          ...
          id: variant.id // Utiliser l'ID du premier variant avec cette couleur
        });
      }
      return acc;
  ...
  const handleAddToBag = () => {
    if (onAddToBag) {
      onAddToBag(selectedVariant || undefined);
    }
  };
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center space-x-2">
            {availableColors.map(({ color, hex, id }) => (
              <button
                key={id} // Utiliser l'ID unique au lieu du nom de couleur
