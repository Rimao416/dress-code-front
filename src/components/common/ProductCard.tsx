...
interface ProductCardProps {
  ...
  onClick?: (product: ProductCardItem) => void; // Changé de Product vers ProductCardItem
  className?: string;
  showBrand?: boolean;
  showPrice?: boolean;
...
const ProductCard: React.FC<ProductCardProps> = ({
  ...
  const handleClick = () => {
    if (onClick) {
      onClick(product); // Passer le produit original
    }
  };
 
  const handleFavoriteClick = (e: React.MouseEvent) => {
    e.stopPropagation();
    onToggleFavorite(normalizedProduct.id); // Utiliser l'ID normalisé
  };
