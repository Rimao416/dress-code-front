  ...
  const FormContent = () => (
    <div className="space-y-6">
      ...
      <div className="space-y-3 pt-2">
        ...
        <p className="text-xs text-neutral-500">
          En créant un compte, j'accepte les <a href="#" className="text-red-900 hover:underline font-medium">Conditions d'utilisation</a>. J'ai lu la <a href="#" className="text-red-900 hover:underline font-medium">Politique de confidentialité</a>.
        </p>
      </div>
      
      <button onClick={submit} disabled={status.loading || status.success} className="w-full bg-gradient-to-r from-red-900 to-red-800 text-white px-6 py-3.5 rounded-md text-sm font-medium shadow-lg hover:shadow-xl transform hover:scale-[1.02] transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed disabled:transform-none flex items-center justify-center gap-2">
        {status.loading ? (
          <><Loader2 className="h-4 w-4 animate-spin" />Inscription en cours...</>
        ) : status.success ? (
          <><CheckCircle className="h-4 w-4" />Inscription réussie !</>
        ) : (
          'Créer un compte'
        )}
      </button>
      ...
      <div className="text-center pt-4 border-t border-stone-200/60">
        <p className="text-sm text-neutral-600">
          ...
          <button onClick={() => { onClose(); onSwitchToLogin?.(); }} className="text-red-900 font-medium hover:underline" disabled={status.loading}>
            Connectez-vous
          </button>
        </p>
      </div>
  ...
  if (isMobile) {
    return (
      <div className="fixed inset-0 z-50 flex items-end">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={status.loading ? undefined : onClose} className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
        <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 30, stiffness: 300 }} className="relative w-full">
          <div className="bg-white rounded-t-3xl overflow-hidden max-h-[90vh] overflow-y-auto">
            <div className="relative bg-gradient-to-br from-stone-50 to-stone-100 p-6 text-center">
              <button onClick={onClose} className="absolute top-4 right-4 text-neutral-400 hover:text-neutral-900" disabled={status.loading}>
                <X className="h-5 w-5" />
              </button>
              <h2 className="text-3xl font-serif text-neutral-900 pt-2">DressCode</h2>
              <div className="flex items-center gap-2 justify-center mt-3">
                <div className="w-8 h-px bg-red-900"></div>
                <h3 className="text-sm font-medium tracking-widest text-red-900 uppercase">Inscription</h3>
                <div className="w-8 h-px bg-red-900"></div>
              </div>
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={status.loading ? undefined : onClose} className="absolute inset-0 bg-black/50 backdrop-blur-sm" />
        <motion.div initial={{ opacity: 0, scale: 0.95, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95, y: 20 }} transition={{ type: "spring", duration: 0.5 }} className="relative bg-white rounded-xl shadow-2xl w-full max-w-5xl h-[650px] overflow-hidden">
          <div className="flex h-full relative">
            <button onClick={onClose} className="absolute top-4 right-4 z-10 text-neutral-400 hover:text-neutral-900" disabled={status.loading}>
              <X className="h-6 w-6" />
            </button>
            ...
            <div className="flex-1 bg-gradient-to-br from-stone-50 to-stone-100 p-8 lg:p-12 flex flex-col justify-center relative overflow-hidden">
              ...
              <div className="relative max-w-md mx-auto space-y-8">
                <div className="space-y-3">
                  <h2 className="text-4xl lg:text-5xl font-serif text-neutral-900">DressCode</h2>
                  <div className="flex items-center gap-3">
                    <div className="w-12 h-px bg-red-900"></div>
                    <h3 className="text-lg font-medium tracking-widest text-red-900 uppercase">Inscription</h3>
                  </div>
                </div>
                <p className="text-neutral-600 text-base">
                  Bienvenue parmi nous ! Profitez de notre livraison gratuite, d'un cadeau pour votre anniversaire, et d'un accès privilégié aux nouveautés.
                </p>
                <div className="inline-flex items-center gap-2 bg-white/90 backdrop-blur-sm px-3 py-1.5 rounded-full shadow-sm border border-stone-200/50">
                  ...
                  <span className="text-xs text-neutral-700 font-medium">Rejoignez notre communauté</span>
                </div>
              </div>
            </div>
